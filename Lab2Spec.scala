...
class Lab2Spec(lab2: Lab2Like) extends FlatSpec {
  ...
  "toNumber" should "convert to a number" in {
    val e1 = B(true)
    val e2 = B(false)
    val e3 = S("words")
    val e4 = S("5")
    val e5 = N(10)
    assert(toNumber(e1)=== 1.0)
    assert(toNumber(e2)=== 0.0)
    assert(toNumber(e3)=== Double.NaN)
    assert(toNumber(e4)=== 5.0)
    assert(toNumber(e5)=== 10.0)
  }

  "toStr" should "convert to a string" in {
    val e1 = N(3)
    val e2 = S("words")
    val e3 = toStr(e1)
    val e4 = B(true)
    val e5 = B(false)
    assert(e3 === "3.0")
    assert(toStr(e2) === "words")
    assert(toStr(e4) === "true")
    assert(toStr(e5) === "false")
  }

  "toBoolean" should "convert to a Boolean" in {
    val e1 = B(true)
    val e2 = B(false)
    val e3 = N(4)
    val e4 = N(-4)
    ...
    assert(toBoolean(e4) === true)
    assert(toBoolean(e5) === false)
    assert(toBoolean(e6) === false)
    assert(toBoolean(e7) === true)
    assert(toBoolean(e8) === false)
